...
let tt =
  ...
  let id_tmp0 = fresh ~next_id:(ref 0) ~loc:loc in
  let id_tmp1 = fresh ~next_id:(ref 1) ~loc:loc in
  let id_tmp2 = fresh ~next_id:(ref 2) ~loc:loc in
  ...
  [
    ...
    begin fun _ ->
      assert_equal (A.Exp.let_
                      Migrate_parsetree.Ast_405.Asttypes.Nonrecursive
                      [ A.Vb.mk
                          (A.Pat.var id_x)
                          test_exp1 ]
                      test_exp2)
    ...
    begin fun _ ->
      ...
      | {pvb_pat = {ppat_desc = Ppat_var {txt = "b_of_a"; _}; _};
         pvb_expr = {pexp_desc =
                       Pexp_fun
                         (Nolabel, None, {ppat_desc = Ppat_var {txt = "u"; _}; _}, matcher_fn);
                     _}; _}
        ->
        ...
        begin match matcher_fn with
          | {pexp_desc =
               Pexp_fun
                 (Nolabel, None,
                  {ppat_desc = Ppat_constraint (_, typ_in); _},
                  {pexp_desc = Pexp_constraint ({pexp_desc = Pexp_match (_, cases); _}, typ_out); _});
             _}
            ->
            ...
            begin match cases with
              | [ (* | `A t0 -> let a = `A t0 in 1 *)
                {pc_lhs = {ppat_desc = Ppat_variant ("A", Some _); _};
                 pc_guard = None;
                 pc_rhs = {pexp_desc =
                             Pexp_let
                               (Nonrecursive, [_],
                                {pexp_desc = Pexp_constant (Pconst_integer ("1", _)); _});
                           _};
                 _};
                (* | `A0 -> 0 *)
                {pc_lhs = {ppat_desc = Ppat_variant ("A0", None); _};
                 pc_rhs = {pexp_desc = Pexp_constant (Pconst_integer ("0", _)); _}; _} ]
                -> ()
    ...
    begin fun _ ->
      ...
      match gen_processor_vb test_L0 test_L0 proc with
      | {pvb_pat = {ppat_desc = Ppat_var {txt = "a_something"; _}; _};
         pvb_expr = {pexp_desc =
                       Pexp_fun
                         (Nolabel, None,
                          {ppat_desc = Ppat_constraint (_, _ (* typ_in*)); _},
                          {pexp_desc = Pexp_match (_, [ _ (* case1 *); _ (* case2 *) ]); _});
                     _};
         _}
        -> ()
      | _ -> assert_failure "processor output invalid"
