...
let tt =
  ...
  [
    ...
    begin fun _ ->
      ...
      | {npnt_name = "expr";
         npnt_productions = [
           {nppr_name = "Two"; nppr_arg = Some (NP_list (NP_tuple _))};
    ...
    begin fun _ ->
      ...
      | NPpat_cata (NPpat_var _, Some {pexp_desc = Pexp_ident {txt = Lident "expr"; _}; _}) -> ()
    ...
    begin fun _ ->
      ...
      | {npc_name = "a";
         npc_dom = dom;
         npc_args = [ (Nanocaml.Ast.Asttypes.Nolabel, _, {ppat_desc = Ppat_var {txt = "x"; _}; ppat_loc = _; ppat_attributes = _});
                      (Nanocaml.Ast.Asttypes.Labelled "y", _, {ppat_desc = Ppat_var {txt = "y"; _}; ppat_loc = _; ppat_attributes = _}) ];
         npc_clauses = clauses;
         _}
        ->
        ...
        begin match pre body with
          | {pexp_desc =
               Pexp_fun (Nolabel, None, (* fun ini -> *)
                         {ppat_desc = Ppat_var {txt = "ini"; _}; _},
                         {pexp_desc =
                            Pexp_let (Nonrecursive, (* let tmp = .. in *)
                                      [ {pvb_pat = {ppat_desc = Ppat_var {txt = "tmp"; _}; _}; _} ],
                                      body');
                          _});
             _}
            ->
